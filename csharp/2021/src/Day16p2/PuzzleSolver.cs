...
public partial class PuzzleSolver
{
    ...
    public long Solve()
    {
        ...
        return Packet.Calculate(Packet.Parse(binary));
    }
...
class Packet
{
    ...
    public static long Calculate(IEnumerable<Packet> packets) =>
        InternalCalculate(packets).First();

    static IEnumerable<long> InternalCalculate(IEnumerable<Packet> packets)
    {
        foreach (var packet in packets)
        {
            ...
            {
                case PacketType.Sum:
                    var sum = InternalCalculate(packet.SubPackets).Sum();
                    yield return sum;
                    break;
                case PacketType.Product:
                    var product = InternalCalculate(packet.SubPackets).Aggregate(1L, (acc, cur) => acc * cur);
                    yield return product;
                    break;
                case PacketType.Minimum:
                    var min = InternalCalculate(packet.SubPackets).Min();
                    yield return min;
                    break;
                case PacketType.Maximum:
                    var max = InternalCalculate(packet.SubPackets).Max();
                    yield return max;
                    break;
                ...
                case PacketType.GreaterThan:
                    {
                        var p1 = InternalCalculate(new[] { packet.SubPackets[0] }).First();
                        var p2 = InternalCalculate(new[] { packet.SubPackets[1] }).First();
                        yield return p1 > p2 ? 1 : 0;
                        break;
                    }
